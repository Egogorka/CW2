...
key_table.forEach( function (value , i , arr) {
    ...
    new_image.src = "/istrolid_clanwars/useful/"+image_table[value];
...
document.querySelector("#mapcode > form").onsubmit = function() 
{
    ...
    AjaxFun('POST','login.php', json, function(responseText) {
        // onreadystate
        alert(responseText);
    }, function() {
        // ontimeout
...
document.getElementById("getmapcode").onclick = function () 
{
    ...
    function Map_Upload() {
        ...
        delete Hexagons_testboard //delete old massive
        Hexagons_testboard = [];  // create new
...
function Hexgrid_Show (){
    ...
    Hexagons_testboard.forEach( function(hextable_x, i) {
...
function Hex_DeleteFromGrid( hex_in , type ) {
    ...
    ThisHex = Hexagons_testboard[xy.col][xy.row];
    if ( type <= 6 ){
        delete Hexagons_testboard[xy.col][xy.row];
    } else {
        ThisHex[1] = 0;
        Hexagons_testboard[xy.col][xy.row] = ThisHex;
    }
}
...
function Hex_AddInGrid( hex_in , type ) {
    ...
    Hxgons_board_col = Hexagons_testboard[xy.col];//[hex_in.r];
    if( Hxgons_board_col === undefined){
        Hexagons_testboard[xy.col] = [];
        Hxgons_board_col = [];
    }
    ...
    Hexagons_testboard[xy.col][xy.row] = ThisHex;
    //alert(Hexagons_testboard[xy.col][xy.row][0] + " | " + Hexagons_testboard[xy.col][xy.row][1]);
}
...
function Hex_Append ( id , offset_coords , typee ) {
        ...
        new_hxgon.id = id;
        new_hxgon.src = "/istrolid_clanwars/useful/"+image_table[key_table[typee]];
...
function Map_Filling_Array( strok ) {
    ...
    mappy.forEach( function( strok_x , i ) {
        ...
        Hexagons_testboard[xy_x] = []; // Добавляем на "карту" столбец Х.
        for (var j = 1; j < hextable_x.length; j++) {
            hextable_y = hextable_x[j].split(";");  //hextable_y будет содержать:
                                                    // ("y", "type", "type")
            Hexagons_testboard[xy_x][Number(hextable_y[0])] = [hextable_y[1], hextable_y[2]];
            //alert( Hexagons_testboard[xy_x][Number(hextable_y[0])] );
        }
    });
    // Функция ничего не выводит, так как работала с переменной Hexagons_testboard, которая, какбы, "глобальная" :D
}
...
function LoginFormTest(form, bool) {
//var str='saafsdfsd23432-sdgfdsg68954&(^)&$)(&)"';
//  alert(str.replace(/[^A-Za-z0-9-_]/g,''));
    if( form.elements.login.value.replace(/[A-Za-z0-9-_]/g,'').length > 0){
        document.getElementById("loginlog").innerHTML = "Login must contain only symbols from english alphabet or numbers";
        return false;               
    }
    if( form.elements.login.value.replace(/[^A-Za-z]/g,'').length < 4){
        document.getElementById("loginlog").innerHTML = "Login must contain at least 4 non-number or non-special symbols";
        return false;               
    }
    if(bool == 1)
        if( !EmailTest(form.elements.email.value)){
            document.getElementById("loginlog").innerHTML = "Wrong email";
            return false;           
        }
    if( form.elements.pswrd1.value.replace(/[A-Za-z0-9-_]/g,'').length > 0){
        document.getElementById("loginlog").innerHTML = "Password must not contain any special-symbols";
        return false;               
    }
    if( form.elements.pswrd1.value.length < 5){
        document.getElementById("loginlog").innerHTML = "Password must have at least 5 symbols";
        return false;       
    }
///    if( form.elements.pswrd1.value != form.elements.pswrd2.value ){
///        document.getElementById("loginlog").innerHTML = "Repeat your password correctly";
///        return false;
///    }
    return true;
}
...
function LoginSignUp(bool) {
    ...
    //alert(formlog.elements.login.value);
    if(!LoginFormTest(formlog, bool)) return false;

    player = [formlog.elements.login.value , formlog.elements.pswrd1.value];
    ...
    AjaxFun('POST','login.php',json, function(responseText) {
        // onreadystate
        document.getElementById("loginlog").innerHTML = responseText;
        if(responseText == "<span style=\"color:green;\">Success</span>") 
        SuccessCall();
    }, function() {
        // ontimeout
...
document.getElementById("loginform").onsubmit = function() {LoginSignUp(0); return false;};
...
function AjaxFun( connectionType , actionScript , json , readyState , timeout) {
    ...
    xhr.onreadystatechange = function() {
        ...
        /*{
            document.getElementById("loginlog").innerHTML = xhr.responseText;
            if(xhr.responseText == "<span style=\"color:green;\">Success</span>") 
                setTimeout( SuccessCall() , 10000 );
        }*/ else
